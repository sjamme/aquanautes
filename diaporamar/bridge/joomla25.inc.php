...
/*
  ...
  Amended by andy syme webmaster@yrc.org.uk to work with J2.5.  USE AT OWN RISK
*/
    ...
    {//bloc for joomla job
        ...
        if ( !defined('_JEXEC'))
            die('<b>Mistake !!!</b> The bridge used doesn\'t match your joomla version!
                (expected Joomla 2.5) <br> Please, correct settings using <a href="bridgemgr.php"> bridge manager </a>');
		...
		//--- _cleanArray depricated in J!2.5 so removed  
...
//------- now let's do a  minimal job with coppermine ------------------------------
    ...
    class cpg_udb extends core_udb {
        function cpg_udb() {
            ...
            //$this->groupstable =  '`' . $this->db['name'] . '`.' . $this->db['prefix'] . $this->table['groups'];  ORIGINAL LINE
			$this->groupstable =  '`' . $this->db['name'] . '`.' . $this->db['prefix'] . $this->table['usergroups'];  // EDITED LINE
            $this->groupmaptable =  '`' . $this->db['name'] . '`.' . $this->db['prefix'] . $this->table['user_usergroup_map'];
        ...
        function collect_groups() {
            ...
            $udb_groups = Array( $this->guestgroup => "Guests", 102 => "Registered" , 103 => "Author", 104 => "Editor", 105 => "Publisher", 106 => "Manager", 108 => "Super Users", 109 => "Club Members", 110 => "Life Members", 111 => "Club Official", 112 => "Site Editor", 113 => "Administrator", );
            return $udb_groups;
        }
        ...
        function authenticate() {
            ...
            if (!$joomUser->get('guest')) { //user connected
             ...
             //   select users.id, user_usergroup_map.group_id as '$t_gid' from users, user_usergroup_map where users.id = user_usergroup_map.group_id group by users.id;
				//$gid=100+$t_gid;					
                $supported_groups = Array(102,103,104,105,106,109,110,111,112,108,113);  //100 + numbers in core_acl_aro_groups table
                if (!in_array($gid,$supported_groups))
                    die("Non standard joomla group used. Sorry this is not supported by the bridge<br>$gid");
        ...
        function login_page() {
            ...
            if (defined('_CB_PROFILER_')) { //cb login page
                ...
                $target ='/index.php?option=com_users&view=login';
        ...
        function logout_page() {
            ...
            if (defined('_CB_PROFILER_')) {//cb logout
                ...
                $target= JRoute::_('index.php?option=com_users&task=user.logout&' . JUtility::getToken() . '=1&return=' . $return);
header('Location: ' . $url);
                $target .='&return=' . base64_encode($script_slash_path);
        ...
        function edit_profile() {
            ...
            if (defined('_CB_PROFILER_')) {
                ...
                $target='/index.php?option=com_users&view=profile&layout=edit';
            }
            $this->redirect($target);
        }
